  ...
  if (markerIntersects.length > 0) {
    ...
    if (marker.userData.isMarker && marker.userData.label) {
      ...
      if (marker.userData.isResearchData) {
        const type =
          marker.userData.type.charAt(0).toUpperCase() +
          marker.userData.type.slice(1);
...
async function fetchSubfields(fieldId) {
  try {
    const response = await fetch(
      `${API_BASE_URL}/subfields?field_id=${fieldId}`
    );
    const data = await response.json();
    return data.data || [];
  } catch (error) {
...
async function fetchFunders(subfieldId) {
  try {
    const response = await fetch(
      `${API_BASE_URL}/funders?subfield_id=${subfieldId}`
    );
    const data = await response.json();
    return data.data || [];
  } catch (error) {
...
async function fetchTopics(funderId, subfieldId) {
  try {
    const response = await fetch(
      `${API_BASE_URL}/topics?funder_id=${funderId}&subfield_id=${subfieldId}`
    );
    const data = await response.json();
    return data.data || [];
  } catch (error) {
...
async function fetchResearchData(filters) {
  try {
    ...
    const response = await fetch(
      `${API_BASE_URL}/research_data?${params.toString()}`
    );
    const data = await response.json();
    return data.data || [];
  } catch (error) {
